...
comments = {
    0x0E00:"tokenised BASIC CALL, start sprites etc",
    ...
    0x2955:"increment searched depth (?)",
    0x2960:"compare searched depth with level (?)",
    0x2962:"if exceeded level, return evaluation (?)",
    0x2964:"else sort moves and recurse (?)",
    0x298D:"compare with level number",
    0x2A0C:"compare score with parent's best, for pruning?",
    ...
    0x2BAD:"find best candidate move", # probably didn't do this deliberately: the address 2BAD can be read as "too bad" i.e. moves not chosen are too bad
    ...
    0x2D50:"move counter (?)",
    0x2D54:"if < 7 moves played, be positional for rooks/queens (?)",
